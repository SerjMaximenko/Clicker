...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function initAutoButtons(i) {
    ...
    const blockBuy = document.createElement('div');
    blockBuy.classList.add('btn');
  ...
  function initLVLUpAutoButtons(i) {
    ...
    async function displayPriceInUSD(ticker) {
      try {
        ...
        span2.textContent = priceInUSD + '$';
      } catch (error) {
        console.error('Ошибка:', error);
        span2.textContent = 'Error fetching price';
      }

      button.appendChild(span2);
    }

    displayPriceInUSD('SBER');
